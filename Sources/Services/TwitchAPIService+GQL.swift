...
extension TwitchAPIService {
    ...
    func fetchLiveStreams(limit: Int = 24) async throws -> [LiveStream] {
        ...
        let response = try JSONDecoder().decode(GQLResponse.self, from: data)
    ...
    // Algorithme "light" pour les Trending VODs
    // Au lieu d'analyser en profondeur les 100 dernières VODs, on va :
    // 1. Récupérer les VODs populaires globales (fallback)
    // 2. Ou récupérer les dernières VODs des channels regardés par l'user (basé sur history)
    func buildPreferenceProfile(history: [PersistentHistoryEntry], watchedVODs: [VOD], subs: [PersistentSubscription]) -> PreferenceProfile {
        var gameScores: [String: Double] = [:]
        var channelScores: [String: Double] = [:]
        var languageScores: [String: Double] = [:]
    ...
    func fetchTrendingVODs(history: [PersistentHistoryEntry], subs: [PersistentSubscription]) async throws -> [VOD] {
        ...
        let profile = buildPreferenceProfile(history: history, watchedVODs: watchedVODs, subs: subs)
            ...
            for login in channelsToFetch {
                group.addTask {
                    ...
                    do {
                        ...
                        let vodResponse = try JSONDecoder().decode(GQLResponse.self, from: vodData)
