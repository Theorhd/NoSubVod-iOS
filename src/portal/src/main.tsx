    ...
    if (url.startsWith('/api/') || url.startsWith('api/')) {
      ...
      if (isTauriRuntime()) {
        const resolvedUrl = url.startsWith('/')
          ...
          : new URL(url.startsWith('api/') ? `/${url}` : url, globalThis.location.origin);
        ...
        const query = resolvedUrl.search ? resolvedUrl.search.slice(1) : undefined;

        return (async () => {
          try {
            const { invoke } = await import('@tauri-apps/api/core');
            const result = await invoke<InternalApiInvokeResponse>('internal_api_request', {
              request: {
                method,
