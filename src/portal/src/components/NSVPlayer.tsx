...
import { usePlayerRecovery } from "./player/usePlayerRecovery";
import {
...
const NSVPlayer = React.memo(
  React.forwardRef(
    (
      ...
      const { attemptRecovery, resetRecoveryWindow } = usePlayerRecovery();
      ...
      const queueSourceRefreshAndResume = useCallback(
        (reason: string) => {
          const now = Date.now();
          if (!attemptRecovery(now)) {
            return false;
          }
        ...
        const intervalId = globalThis.setInterval(() => {
          ...
          try {
            ...
            isNativeFullscreen = Boolean(
              activeVideo?.webkitDisplayingFullscreen,
            );
          } catch {
            // Ignore
          }
      ...
      useEffect(() => {
        ...
        const intervalId = globalThis.setInterval(() => {
          ...
          const queued = queueSourceRefreshAndResume(
            "native-vod-token-rotation",
          );
          if (queued) {
      ...
      }, [source.src, source.type, resumeRevision, resetRecoveryWindow]);
